...
public class TestIcsParser extends TestSuite {
    ...
    public void parseIcsFile_allBeginVeventMissing_noEventsProcessed()
        ...
        ArrayList<WorkSession> testCalendar = IcsParser
                .parseIcsFile("./test/testdata/allBeginVeventMissing.ics");
    ...
    public void parseIcsFile_endVCalendarMissing_allEventsProcessedNoException()
            throws WaktuException {
        ArrayList<WorkSession> testCalendar = IcsParser
                .parseIcsFile("./test/testdata/endVCalendarMissing.ics");
    ...
    public void parseIcsFile_Normal() throws WaktuException {
        ArrayList<WorkSession> testCalendar = IcsParser
                .parseIcsFile("./test/testdata/normal.ics");
        assertCalendarValidity(testCalendar, 3);
    }
    ...
    public void parseIcsFile_invalidTimeFormat_onlyValidEventsProcessed()
            throws WaktuException {
        ArrayList<WorkSession> testCalendar = IcsParser
                .parseIcsFile("./test/testdata/invalidTimeFormat.ics");
    ...
    public void parseIcsFile_startTimeEndTimeMissing_onlyValidEventsProcessed()
            throws WaktuException {
        ArrayList<WorkSession> testCalendar = IcsParser
                .parseIcsFile("./test/testdata/startTimeEndTimeMissing.ics");
        assertCalendarValidity(testCalendar, 1);
    }
    ...
    public void parseIcsFile_summaryMissing_onlyValidEventsProcessed()
            throws WaktuException {
        ArrayList<WorkSession> testCalendar = IcsParser
                .parseIcsFile("./test/testdata/summaryMissing.ics");
        assertCalendarValidity(testCalendar, 2);
    }

    @Test(expected = WaktuException.class)
    public void parseIcsFile_invalidFilepathInArgument_NullReturnedWaktuGeneralException()
            throws WaktuException {
        ArrayList<WorkSession> testCalendar = IcsParser.parseIcsFile(null);
        assertEquals(null, testCalendar);
    }

    private void assertCalendarValidity(
            final ArrayList<WorkSession> testCalendar,
            final int noOfValidEntries) {
        for (int i = 0; i < noOfValidEntries; i++) {
            assertEquals(calendar.get(i).getDescription(), testCalendar.get(i)
                    .getDescription());
        }
        for (int i = 0; i < noOfValidEntries; i++) {
            assertEquals(calendar.get(i).getStart(), testCalendar.get(i)
                    .getStart());
        }
        for (int i = 0; i < noOfValidEntries; i++) {
