...
public class TableSortFilterModel extends QSortFilterProxyModel {
	...
	protected boolean filterAcceptsRow(int sourceRow, QModelIndex sourceParent) {
		...
		if (project != null) {
			matchFound = model.data(idxProj).toString()
					.equals(project.toString());
		}
		if (workPackage != null) {
			matchFound = matchFound
					&& model.data(idxWP).toString()
							.equals(workPackage.toString());
		}
		if (usr != null) {
			matchFound = matchFound
		...
		if (start != null) {
			matchFound = matchFound && start.compareTo(currStart) <= 0
					&& start.compareTo(currEnd) >= 0;
		}
		if (end != null) {
			matchFound = matchFound && end.compareTo(currStart) >= 0
