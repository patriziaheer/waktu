...
public class TimeController {
	...
	public static double calculateWorktime(Usr usr, Project project, WorkPackage workPackage, QDate start, QDate end) throws WaktuException {
		...
		try {
			for (WorkSession ws : workSessions) {
				if ((workPackage == null || ws.getWorkPackage().equals(workPackage))
					&& (start == null || start.compareTo(TimeUtil.convertGregorianToQDateTime(ws.getStart()).date()) <= 0)
					&& (end == null || end.compareTo(TimeUtil.convertGregorianToQDateTime(ws.getEnd()).date()) >= 0)) {
	...
	private static double calculateWorktime(Project project, QDate start, QDate end) throws WaktuException {
		double worktime = 0;
	...
	private static double calculateWorktime(Project project, WorkPackage workPackage) throws WaktuException {
		double worktime = 0;
	...
	private static double calculateWorktime(Project project, WorkPackage workPackage, Usr usr) throws WaktuException {
		double worktime = 0;

		try {
			for (WorkSession ws : WorkSessionController.getInstance().getWorkSessions(workPackage, usr)) {
				worktime += TimeUtil.calculateTimespanInSeconds(ws.getStart(), ws.getEnd());
			}
		} catch (NullPointerException e) {
			return 0;
	...
	public static double calculateOvertime(Usr user, QDate fromDate, QDate toDate) throws WaktuException {
		return calculateWorktime(user, null, null, fromDate, toDate) - getPlannedTime(user, fromDate, toDate);
	}
	...
	public static double calculateWorktimeForWeek(Usr user, QDate date) throws WaktuException {
		...
		return calculateWorktime(user, null, null, weekStartDateEndDate[0], weekStartDateEndDate[1]);
	}
	...
	public static double calculateWorktimeForMonth(Usr user, QDate date) throws WaktuException {
		...
		return calculateWorktime(user, null, null, monthStartDateEndDate[0], monthStartDateEndDate[1]);
	}
	...
	private static double getNumberOfWorkdaysForMonth(Usr user, QDate currMonth) {
		return user.getPensum() / 100.0
				* getNumberOfWorkdays(user, TimeUtil.getMonthBoundaries(currMonth)[0], TimeUtil.getMonthBoundaries(currMonth)[1]);
	}
