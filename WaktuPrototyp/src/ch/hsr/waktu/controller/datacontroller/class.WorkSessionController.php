...
class WorkSessionController {
	...
	public function getAllWorkSession() {
		...
		$query = pg_query($this->con, "SELECT to_char(starttime,'DD.MM.YYYY HH24:MI') as starttimef, to_char(endtime,'DD.MM.YYYY HH24:MI') as endtimef, * FROM worksession ORDER BY id DESC LIMIT 6;");
		
		$counter = 0;
		
		while($data = pg_fetch_object($query)) {
			$workSession[$counter] = new WorkSession(UsrController::getInstance()->getUsr($data->usr), $data->endtimef, $data->starttimef, WorkPackageController::getInstance()->getWorkPackage($data->workpackage), $data->description, $data->id);
			$counter++;
		}
